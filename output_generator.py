...
class OutputGenerator:
    ...
    def _format_json(self, processed_results):
        ...
        """
        # For JSON, we might want to strip the >>> <<< from the snippet for cleaner data
        # or add a separate field for the raw line vs highlighted snippet.
    ...
    def _format_markdown(self, processed_results):
        ...
        for result in processed_results:
            ...
            output_lines.append("```text") # Using text for snippet to preserve formatting from >>> <<<
            output_lines.append(result['snippet'])
            output_lines.append("```")

            if result.get('elaboration'):
                output_lines.append("\n**Elaboration:**")
                output_lines.append("> " + result['elaboration'].replace('\n', '\n> ')) # Blockquote for elaboration
...
if __name__ == '__main__':
    ...
    print(json_output)
    # To save to file: 
    # with open("test_output.json", "w") as f: f.write(json_output)
    ...
    print(md_output)
    # To save to file: 
    # with open("test_output.md", "w") as f: f.write(md_output)
