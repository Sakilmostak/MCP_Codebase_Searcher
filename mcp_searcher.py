...
from report_elaborator import elaborate_finding # Added import
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="MCP Codebase Searcher: Searches codebases and elaborates on findings.", # Updated description
    ...
    # All existing parser.add_argument calls for search will become search_parser.add_argument
    ...
    # REMOVED the old -e/--elaborate flag from search_parser
    ...
    # This section for handling ELABORATE_AVAILABLE and the old -e flag is no longer needed here as -e is removed.
    ...
    # REMOVE THE VALIDATION FROM HERE
    # if args.context < 0:
...
def main():
    ...
    if args.command == 'search':
        ...
        if args.context < 0:
            # Need to get the search_parser instance to call error on it
            # This is tricky. A simpler way is to raise ValueError or print and exit.
            # For consistency with argparse, let's find a way or adjust.
            # Alternative: access parser from main scope if it was returned by parse_arguments, 
            # or re-create just the search_parser for the error message.
            # Simpler: print to stderr and exit, like other custom errors.
        ...
        except Exception as e: # Catch-all for unexpected errors in search command
        ...
        try:
            ...
            if elaboration_result.startswith("Error:"):
                sys.exit(1) # Exit with error if elaborate_finding returned an error string
        except Exception as e: # Catch-all for unexpected errors in elaborate command
